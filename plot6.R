...
# with vertical bars would seem to be the best choice.
            ...
            select(fips,year,Emissions) %>%
                group_by(fips,year) %>%
                    summarize(emissions = sum(Emissions)) %>%
                        mutate(county = ifelse(fips == "24510", "Baltimore, MD", "Los Angeles, CA"))

g <- ggplot(df, aes(x=factor(year), y=emissions, fill=county)) +
     geom_bar(stat="identity") +
     facet_grid(county ~ ., scales = "free") +
     guides(fill = FALSE) +
     labs(x="Year") +
     labs(y=expression("Total PM"[2.5]*" Emissions ( In Tons )")) +
     labs(title=expression("PM"[2.5]*" Motor Vehicle Emissions From 1999 - 2008"))
